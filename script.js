...
const balancedTeams = () => {
  ...
  let teams = new Array(parseInt(numberOfTeams)).fill(null).map(item => []);
  ...
  toShuffle.forEach((player, index) => teams[index % numberOfTeams].push(player));
    ...
    arr.forEach(function (item) {
      ...
      item.forEach(function (object) {
        if (object.teamSkill) {
          ...
          elementP += `<span class="name liststyle">${object.name} ${object.skill}</span>`;
        }
      });
